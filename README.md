...
A behavior is a piece of semi-arbitrary Java code that an organism executes once per turn.  The constraints on the behavior consist of a limited number of inputs (primarily information about the world around the organism and its current location) and at least one output specifying which behavior should follow.  Now, the behavior does not have access to what the other possible behaviors are, merely to their indices.  The one other thing necessary for a behavior is a method to mutate the behavior.
